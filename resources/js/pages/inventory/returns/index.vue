...
const load = async () => {
  const payload = await $api('/inventory/returns', { query: { page: page.value } })
...
<template>
  <div>
    ...
    <HCard flush>
      <HTable
        :loading="pending"
        :headers="[{ title: 'Return', key: 'reference' }, { title: 'From', key: 'from_store.name' }, { title: 'To', key: 'to_store.name' }, { title: 'When', key: 'occurred_at' }]"
        :items="rows"
        empty="No returns"
      >
