...
class ReportSnapshot
{
    public static function make(User $user): array
    {
        ...
        $patientCounts = self::countsBy(Patient::query(), 'status');
        $encounterStatus = self::countsBy(Encounter::query(), 'status');
        $openByType = Encounter::query()
            ->select('type', DB::raw('COUNT(*) as aggregate'))
            ->whereIn('status', ['waiting', 'in_progress'])
            ->groupBy('type')
            ->pluck('aggregate', 'type');
        $invoiceCounts = self::countsBy(Invoice::query(), 'status');
        $referralCounts = self::countsBy(Referral::query(), 'status');
        $assistanceCounts = self::countsBy(AssistanceRequest::query(), 'status');
        $ambulanceCounts = self::countsBy(Ambulance::query(), 'status');
    ...
    private static function countsBy($query, string $column): array
    {
        return (clone $query)
